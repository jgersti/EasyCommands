...
namespace IngameScript {
    partial class Program {
        ...
        void AddBiOperation<T, U>(BiOperand type, Func<T, U, object> resolver) {
            foreach (var key in GetTypes(typeof(T)).SelectMany(t => GetTypes(typeof(U)), (t, u) => MyTuple.Create(type, t, u)))
                BiOperations[key] = (p, q) => ResolvePrimitive(resolver((T)p.value, (U)q.value));
        }
